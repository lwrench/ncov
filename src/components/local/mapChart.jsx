...
class MapChartLocal extends Component {
    ...
    deal=(citys,location)=>{
        ...
        privince.forEach(value=>{
            allcitys.set(value.provincename,value.city)
        })

        const currects=allcitys.get(location)
        // console.log("currect",currects)

        const currectcity=[]
        citys.forEach(city=>{
            if(city!==null){
                let name=city.name.substring(0,2)
                let value=city.value
                for(let i=0;i<currects.length;i++){
                    if (currects[i].indexOf(name)!==-1){
                        name=currects[i]
                        break;
                    }
                    // console.log(i,name)
                }
                currectcity.push({"name":name,"value":value})
            }
        })

        // console.log('currectcity',currectcity)
        return currectcity
    }
